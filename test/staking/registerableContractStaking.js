...
async function baseTestingCases(owner, client, stakingContract, mintableToken, tokenPairs, plans) {
    ...
    describe("Settings", async function () {
        ...
        it("should be able to set a simple plan", async function () {
            const planId = await stakingContract.callStatic.setPlan(1, 10 * 10 ** 8, 10, 0);
            planSetReceipt = await stakingContract.setPlan(1, 10 * 10 ** 8, 10, 0);
            plans.push(planId);
        });
        it("should be able to get an event for setting the plan", async function () {
            ...
            expect(eventData.apy).to.be.equal(10 * 10 ** 8);
        ...
        it("should be able to reactivate the plan", async function () {
            await stakingContract.setPlan(1, 10 * 10 ** 8, 10, 0);
    ...
    describe("Stake and unstake", async function () {
        ...
        it("should be able to return stake event", async function () {
            ...
            expect(eventData.pips.toNumber()).to.be.equal(8);
        ...
        it("should be able to unstake if the time has passed the staking period", async function () {
            ...
            //const totalSupply = new BigNumber((await mintableToken.totalSupply()).toString());
            ...
            //expect((await mintableToken.totalSupply()).toString()).to.be.equal((totalSupply.plus(27397)).toString());
        });
        it("should be able to return correct current TVL", async function () {
    ...
    describe("Stake, withdraw/unstakTo/withdrawTo", async function () {
        let stakeId;
        before(async function () {
            plans.push(await stakingContract.callStatic.setPlan(2, 10 * 10 ** 8, 10, 0));
            planSetReceipt = await stakingContract.setPlan(2, 10 * 10 ** 8, 10, 0);
            const signerBalance = await mintableToken.balanceOf(owner.getAddress());
            await mintableToken.approve(stakingContract.address, 100000000);
    ...
    describe("Stake (compound) and combined, check stake status externally and emergencyExit", async function () {
        ...
        before(async function () {
            ...
            plans.push(await stakingContract.callStatic.setPlan(4, 10 * 10 ** 8, 10, 200000000));
            planSetReceipt = await stakingContract.setPlan(4, 10 * 10 ** 8, 10, 200000000);
        ...
        it("Should be able to stake", async function () {
            ...
            expect(awaitedReceipt2.events[awaitedReceipt2.events.length - 1].event).to.be.equal("TopUp");
            const eventData2 = stakingContract.interface.decodeEventLog(
                "TopUp",
                awaitedReceipt2.events[awaitedReceipt2.events.length - 1].data,
            );
            expect(eventData2.staked).to.be.equal(300027397);
        ...
        it("should be able to return correct current Issued Tokens and Expected Tokens to issue", async function () {
            ...
            expect((await stakingContract.getExpectedIssuedTokens()).toNumber()).to.be.equal(816772); //487,804 + 465,961
        });
        it("Should be able to emergency exit after a day has passed and emit event", async function () {
            await network.provider.send("evm_mine", [timestamp + 86400 * 8 + 100000]);
            const stake = (await stakingContract.getStakes())[0];
            const plan = await stakingContract.getPlan(stake.planId);
            const planNew = await stakingContract.getPlan(plans[2]);
            console.log("Times");
            console.log(stake.depositTime);
            console.log(stake.depositTime.toNumber() + planNew.period.toNumber() * 86400);
            const emergencyExitParameters = await stakingContract.callStatic.emergencyExit(stakeId);
            const staked = 300000000 + 100000000 + 27397;
            const rewards = Math.floor(staked / 3650);
            const emergencyLoss = Math.floor(((staked + rewards) * plan.emergencyTax) / 100);
            expect(emergencyExitParameters.emergencyLoss).to.be.equal(emergencyLoss + 3);
            ...
            expect(emergencyExitParameters.withdrawn.toNumber()).to.be.lessThanOrEqual(
                staked + rewards - emergencyLoss + 27,
            );
            ...
            expect(eventData.withdrawn.toNumber()).to.be.lessThanOrEqual(staked + rewards - emergencyLoss + 27);
            expect(eventData.planId).to.be.equal(plans[2]);
            expect(eventData.timestamp.toNumber()).to.be.equal(stake.depositTime.toNumber());
        });
        it("should be able to return correct current TVL", async function () {
            expect((await stakingContract.getTVL()).toNumber()).to.be.equal(300000000); //400,027,397 + 300000000
        });
        it("should be able to return correct current Issued Tokens and Expected Tokens to issue", async function () {
            expect((await stakingContract.getIssuedTokens()).toNumber()).to.be.equal(257568); //131507 + 27397
            expect((await stakingContract.getExpectedIssuedTokens()).toNumber()).to.be.equal(586468); //487,804 + 465,961
        });
        it("Should there be only 1 stake left", async function () {
            ...
            console.log(
                (
                    await stakingContract.getStakeRewards(
                        stakes[0].stakeId,
                        timestamp + 86400 * 6 + 60000,
                        timestamp + 86400 * 8 + 100000,
                    )
                ).toString(),
            );
            console.log(
        ...
        it("should be able to return correct current Issued Tokens and Expected Tokens to issue", async function () {
            expect((await stakingContract.getIssuedTokens()).toNumber()).to.be.equal(405533); //131507 + 27397
            expect((await stakingContract.getExpectedIssuedTokens()).toNumber()).to.be.equal(405533); //487,804 + 465,961
        });
    });
    describe("Stake (compound), withdraw and unstake with non-whole number APY", async function () {
        ...
        before(async function () {
            ...
            plans.push(await stakingContract.callStatic.setPlan(12, 12.02046 * 10 ** 8, 10, 0));
            await stakingContract.setPlan(12, 12.02046 * 10 ** 8, 10, 0);
        ...
        it("should be able to return correct current Issued Tokens and Expected Tokens to issue", async function () {
            expect((await stakingContract.getIssuedTokens()).toNumber()).to.be.equal(405533); //131507 + 27397
            expect((await stakingContract.getExpectedIssuedTokens()).toNumber()).to.be.equal(801437); //487,804 + 465,961
        });
        ...
        it("should be able to return correct current Issued Tokens and Expected Tokens to issue", async function () {
            expect((await stakingContract.getIssuedTokens()).toNumber()).to.be.equal(801437); //131507 + 27397
            expect((await stakingContract.getExpectedIssuedTokens()).toNumber()).to.be.equal(801437); //487,804 + 465,961
        });
    ...
    describe("Stake (non-compound), withdraw and unstake with non-whole number APY", async function () {
        let stakeId;
        before(async function () {
            //await network.provider.send("evm_mine", [timestamp + 86400 * 24 + 10000]);
            plans.push(await stakingContract.callStatic.setPlan(730, 10 * 10 ** 8, 10, 0));
            await stakingContract.setPlan(730, 10 * 10 ** 8, 10, 0);
            const signerBalance = await mintableToken.balanceOf(owner.getAddress());
            await mintableToken.approve(stakingContract.address, 10000000000);
            const stakeReceipt = await stakingContract.stake(100000000, plans[plans.length - 1], false);
            expect(await mintableToken.balanceOf(owner.getAddress())).to.be.equal(signerBalance - 100000000);
            const awaitedReceipt = await stakeReceipt.wait();
            expect(awaitedReceipt.events[awaitedReceipt.events.length - 1].event).to.be.equal("Staked");
            const eventData = stakingContract.interface.decodeEventLog(
        ...
        it("should be able to return correct current Issued Tokens and Expected Tokens to issue", async function () {
            expect((await stakingContract.getIssuedTokens()).toNumber()).to.be.equal(801437); //131507 + 27397
            expect((await stakingContract.getExpectedIssuedTokens()).toNumber()).to.be.equal(20801437); //487,804 + 465,961
        });
        it("should be able to get the correct expected revenue", async function () {
            const revenue = await stakingContract.expectedRevenue(
                100000000,
                plans[plans.length - 1],
                false,
                Math.floor(timestamp + 86400 * 24 + 10000),
                Math.floor(timestamp + 86400 * 754 + 20000),
            );
            expect(revenue.toNumber()).to.be.equal(20000000);
            //check -  https://www.thecalculatorsite.com/finance/calculators/daily-compound-interest.php
        });
        it("should be able to withdraw the full amount of rewards", async function () {
            await network.provider.send("evm_mine", [timestamp + 86400 * 755 + 20000]);
            const balance = await mintableToken.balanceOf(owner.getAddress());
            const awaitedReceipt = await (await stakingContract.withdrawRewards(stakeId)).wait();
            expect((await mintableToken.balanceOf(owner.getAddress())).toNumber()).to.be.equal(
                balance.toNumber() + 20000000,
            );
            expect(awaitedReceipt.events[awaitedReceipt.events.length - 1].event).to.be.equal("RewardsWithdrawn");
            const eventData = stakingContract.interface.decodeEventLog(
                "RewardsWithdrawn",
            ...
            expect(eventData.rewards.toNumber()).to.be.equal(20000000);
            expect(eventData.planId).to.be.equal(plans[plans.length - 1]);
            expect(eventData.stakeId).to.be.equal(stakeId);
            expect(eventData.amount.toNumber()).to.be.equal(100000000);
        });
        ...
        it("should be able to return correct current Issued Tokens and Expected Tokens to issue", async function () {
            expect((await stakingContract.getIssuedTokens()).toNumber()).to.be.equal(20801437); //131507 + 27397
            expect((await stakingContract.getExpectedIssuedTokens()).toNumber()).to.be.equal(20801437); //487,804 + 465,961
        });
        it("should be able to unstake the correct amount", async function () {
            const balance = await mintableToken.balanceOf(owner.getAddress());
    ...
    describe("Stake (non-compound), withdraw and unstake with non-whole number APY", async function () {
        ...
        it("should be able to return correct current Issued Tokens and Expected Tokens to issue", async function () {
            expect((await stakingContract.getIssuedTokens()).toNumber()).to.be.equal(20801437); //131507 + 27397
            expect((await stakingContract.getExpectedIssuedTokens()).toNumber()).to.be.equal(40801437); //487,804 + 465,961
        });
        it("should be able to get the correct expected revenue", async function () {
            const revenue = await stakingContract.expectedRevenue(
                100000000,
                plans[plans.length - 1],
                false,
                Math.floor(timestamp + 86400 * 755 + 20000),
                Math.floor(timestamp + 86400 * 1485 + 30000),
            );
            expect(revenue.toNumber()).to.be.equal(20000000);
            //check -  https://www.thecalculatorsite.com/finance/calculators/daily-compound-interest.php
        });
        it("should be able to emergency exit", async function () {
            await network.provider.send("evm_mine", [timestamp + 86400 * 1120 + 60000]);
            const balance = (await mintableToken.balanceOf(owner.getAddress())).toNumber();
            const stake = (await stakingContract.getStakes())[0];
            const plan = await stakingContract.getPlan(stake.planId);
            const emergencyExitParameters = await stakingContract.callStatic.emergencyExit(stakeId);
            const staked = 100000000;
            ...
            const emergencyLoss = Math.floor(((staked + rewards) * plan.emergencyTax) / 100);
            expect(emergencyExitParameters.emergencyLoss).to.be.equal(emergencyLoss);
            expect(emergencyExitParameters.withdrawn.toNumber()).to.be.equal(
                staked + rewards - emergencyLoss
            );
            const awaitedReceipt = await (await stakingContract.emergencyExit(stakeId)).wait();
            const eventData = stakingContract.interface.decodeEventLog(
                "EmergencyExited",
            ...
            expect(eventData.staked).to.be.equal(staked);
            expect(eventData.withdrawn.toNumber()).to.be.equal(staked + rewards - emergencyLoss);
            expect(eventData.planId).to.be.equal(stake.planId);
            expect(eventData.timestamp.toNumber()).to.be.equal(stake.depositTime.toNumber());
            expect((await mintableToken.balanceOf(owner.getAddress())).toNumber())
                .to.be.equal(balance +staked + rewards - emergencyLoss);
        });
        it("should be able to return correct current TVL", async function () {
            expect((await stakingContract.getTVL()).toNumber()).to.be.equal(0); //400,027,397 + 300000000
        });
        it("should be able to return correct current Issued Tokens and Expected Tokens to issue", async function () {
            expect((await stakingContract.getIssuedTokens()).toNumber()).to.be.equal(29801437); //131507 + 27397
            expect((await stakingContract.getExpectedIssuedTokens()).toNumber()).to.be.equal(29801437); //487,804 + 465,961
        });
    });
}
